...
def Model():
    ...
    try:
        TPd = float(input('Enter Your Desired Throuput Btween 280 and 5650 kg/hr\t'))
        assert 280 <= TPd <= 5650
    ...
    VR2 = 9                                                             # speed ratio of digester speed reducer
    VR6 = 6                                                             # speed ratio of Screw Press speed reducer
    ...
    Ao = np.array([0.02134, 0.02667, 0.03340, 0.0483, 0.0603, 0.0889, 0.1143, 0.1413, 0.1683, 0.2191, 0.273, 0.3239, 0.3556, 0.4064])

    # ==== Internal Diameters of Digester Barrel (Pi)in m ====
    Pi = np.array([0.15, 0.20, 0.25, 0.30, 0.35, 0.40])

    # ==== External Diameters of auger membrane (Po)in m ====
    ...
    A = np.array([645, 696, 747, 823, 848, 925, 950, 1001, 1026, 1051, 1102, 1128, 1204, 1255, 1331, 1433, 1458, 1509,
                  1560,1636, 1661, 1687, 1763, 1814, 1941, 2017, 2068, 2093, 2195, 2322, 2474, 2703, 2880, 3084, 3287, 3693])
    ...
    D = np.array([3127, 3330, 3736, 4092, 4194, 4473, 4651, 5032, 5413, 6124, 6886, 7648, 8410, 9172, 9934, 10696, 12220, 13744, 15268, 16792])

    E = np.array([5426, 6137, 6899, 7661, 8423, 9185, 9947, 10709, 12233, 13757, 15283, 16805])

    BeltType = dict()

    # CONSTANTS PARAMETERS FOR THE INTEGRATED MACHINE
    ...
    c1 = 0.01                                                   # clearance between the cake breaker beaters and the separtor top cover, in m
    ...
    wab = 0.0025                                                # width of the auger screw flihgt(i.e. depth of the auger screw), in m
    ...
    Dmp = 0.176  # diameter of the electric motor pulley, in m
    ...
    ddi = (((16 * Td1) / (pi * tau1)) ** (1 / 3)) / 1e3 # Calculated inner diameter of digester membrane/Bore diameter of digester Pulley, in m
    ...
    Mdb = ((Wd / L2) * (Ldb)) * 1e3  # Maximum bending moment on each of the digester beater based on point load/cantilever support,in N
    ...
    O1 = (pi-(2*(asin((Ddp1-Dmp)/(2*C1))))) #angle of lap for the electric motor/digester speed reducer drive, in radians
    T2 = T1/exp((u*O1)/(sin(B2))) #Tension on the slack side of electric motor/digester speed reducer drive belt, in N
    ...
    O2 = (pi-(2*(asin((Ddp2-Dcp)/(2*C2)))))  #angle of lap on the Small pulley of digester/cakebreaker belt drive, in radians
    T4 = T3/exp((u*O2)/(sin(B2))) #Tension on the slack side of digester/cakebreaker drive belt, in N
    Lb1=((2*C1)+((pi/2)*(Ddp1+Dmp))+(((Ddp1-Dmp)**2))/(4*C1))*1000  #theorectical pitch length of electric motor/digester speed reducer drive belt, in m
    ...
    RD = (((-W1) * L1) + (W2 * (L2 / 2)) + (W3 * (L1 + L2))) / L2  # Axial load on the RHS support (Bearing 2) of the digester shaft, in N
    ...
    A1, T1, e, f, Belt_Type, Belt = BeltSelector(Pc1)
    ...
    Lb2 = (2 * C2) + ((pi / 2) * (Ddp2 + Dcp)) + (((Ddp2 - Dcp) ** 2)) / (4 * C2)
    Lb2 = min(Belt[Belt >= Lb2])
    ...
    Tc = (((Mbc * kb) ** 2 + (Mtc * kt) ** 2) ** (1 / 2)) * 1e3  # equivalent Twisting moment on the cakebreaker shaft,in N-mm
    ...
    Pa1 = Pa * sf
    A1, T1, e, f, Belt_Type, Belt = BeltSelector(Pa1)
    ...
    Lb3 = (2 * C3) + ((pi / 2) * (Dap + Dcp)) + (((Dap - Dcp) ** 2)) / (4 * C3)  # theorectical pitch length of the cake breaker/auger drive belt, in m
    ...
    Lb4 = (2 * C4) + ((pi / 2) * (Dap + Drp)) + (((Dap - Drp) ** 2)) / (4 * C4)  # theorectical pitch length of the auger/screw press drive belt, in m
    Lb4 = min(Belt[Belt >= Lb4])
    ...
    msf = ps * Vsf  # Mmass of the screw flight, in kg
    ...
    Ts = (((Mbs * kb) ** 2 + (Mtp1 * kt) ** 2) ** (1 / 2)) * 1e3  # equivalent Twisting moment on the screw press shaft, in kN-mm
    ds1 = ((16 * (Ts)) / (0.27 * pi * do)) ** (1 / 3)  # diameter of screw press shaft, in mm

    # SELECTION OF THE SPEED REDUCERS
    Pds = ((2 * pi * Nd * (Td1 / 1000)) / 60) / 746  # Capacity of the speed reducer required for the digester unit, in horse power
    ...
    DigesterParameters = {'Number of batches for the digestion operation per hour': [nd, 'batch/hr'], 'Maximum mass capacity of the digester barrel, in kg/batch': [md, 'kg/batch'], 'Standard inner diameter of digester membrane': [Di, 'm'], 'Standard outer diameter of digester membrane': [Do, 'm'], 'Standard inner diameter of digester barrel': [Ddi, 'm'], 'Pitch of the digester beaters': [Ldmp, 'm'], 'Length of digester discharge opening': [wd, 'm'], 'Width of digester discharge opening': [Bd, 'm'], 'length of digester beaters': [Ldb, 'm'], 'Length of digester and cake breaker membrane': [Lm, 'm'], 'Number of digester beaters': [ndb, ''], 'Standard diameter of digester beaters': [Do1, 'm'], 'Length of LHS Extension of digester shaft': [L1, 'm'], 'Length of RHS Extension of digester shaft': [L3, 'm'], 'Length of digester shaft': [Ld, 'm'], 'Diameter of the digester shaft based on shear stress': [dd, 'mm'], 'Diameter of the digester shaft based on bending stress': [DD, 'mm']}
    
    SeparatorParameters = {'length of auger membrane': [Lam, 'm'], 'length of the separator pulp discharge chute': [La, 'm'], 'Standard outer diameter of auger membrane': [dam, 'm'], 'Standard internal diameter of auger membrane': [dami, 'm'], 'Pitch of helix on the auger membrane': [pam, 'm'], 'Flow velocity of palm nuts towards its discharge opening': [vn, 'm/s'], 'External diameter of cake breaker membrane': [Dco, 'mm'], 'Length of the cake breaker beaters': [Lcb, 'm'], 'Number of cake breaker beaters': [ncb, ''], 'Diameter of cake breaker beaters': [Dcb, 'm'], 'Inner diameter of cake breaker membrane': [Dci, 'mm'], 'Effective length of the separator chamber top': [a, 'm'], 'Effective length of the separator chamber base': [b, 'm'], 'Height of digester discharge chute': [hd, 'm'], 'Distance of discharge chute of the digester': [L4, 'm'], 'Diameter of the cake breaker shaft based on shear stress': [dc, 'mm'], 'Diameter of the cake breaker shaft based on bending stress': [DC, 'mm'], 'number helix on the auger conveyor membrane': [nab, ''], 'Diameter of the Auger shaft based on shear stress': [da, 'mm'], 'Diameter of the Auger shaft based on bending stress': [DA, 'mm']}

    ScrewPressParameters = {'Internal diameter of screw press barrel': [Dsf, 'm'], 'Screw Pitch': [s, 'm'], 'depth of the screw flight': [dcs, 'm'], 'diameter of the screw shaft': [ds, 'm'], 'mean thread diameter': [Dm, 'm'], 'length of the barrel': [Lp, 'm'], 'number of screw turns': [ns, ''], 'helix angle of the screw': [Af, 'degrees'], 'screw channel width': [wc, 'm'], 'flight land width': [tf, 'm'], 'diameter of screw press shaft': [ds1, 'mm']}
    
    PulleyParameters = {'Diameter of the pulley on the digester speed reducer': [Ddp1, 'm'], 'Diameter of the digester driving pulley': [Ddp2, 'm'], 'Diameter of the cake breaker pulley': [Dcp, 'm'], 'Diameter of auger Auger driven pulley': [Dap, 'm'], 'Diameter of screw press speed reducer pulley': [Drp, 'm'], 'Diameter of screw press pulley (if speed reducer is not used)': [Drp1, 'm']}
    
    BeltParameters = {'Minimum center distance between electric motor and digester shafts': [C1, 'm'], 'Minimum center distance between Digester and cake breaker shafts': [C2, 'm'], 'Minimum center distance between cake breaker and auger shafts': [C3, 'm'], 'Minimum center distance between auger and screw press shafts': [C4, 'm'], 'pitch length of electric motor/digester speed reducer drive belt': [Lb1, 'm'], 'pitch length of the digester/cake breaker drive belt': [Lb2, 'm'], 'pitch length of the cake breaker/auger drive belt': [Lb3, 'm'], 'pitch length of the auger/screw press drive belt':[Lb4, 'm'], 'Actual Number of belts for the electric motor/speed reducer drive': [N1a, ''], 'Actual Number of belts for the digester/cake breaker drive': [N2a, ''], 'Actual Number of belts for the cake breaker/auger drive': [N3a, ''], 'Actual Number of belts for the auger/speed reducer drive': [N4a, '']}
   
    BeltParameters.update(BeltType)
   
    PowerRequirement = {'Capacity of the speed reducer required for the digester unit': [Pds, 'hp'], 'Capacity of the speed reducer required for the screw press': [Pps, 'hp'], 'Total power required to drive the integrated machine': [Pt, 'W'], 'Capacity of the required electric motor (accounting for 10%loss)': [PkW, 'W'], 'Capacity of the required electric motor, in horse power': [Php, 'hp']}
   
    
    Parameters = {'Parameters for Digester Unit':DigesterParameters, 'Parameters for Seperator Unit':SeparatorParameters, 'Parameters for Screw press unit':ScrewPressParameters, 'Parameters of the Pulley System':PulleyParameters, 'Parameters of the drive System':BeltParameters, 'Power Requirement of the Machine':PowerRequirement}



def Display():
    Model()
    Title = 'DESIGN PARAMETERS OF INTEGRATED PALM OIL PROCESSING MACHINE AT ' + str(TPd) + ' KILOGRAM PER HOUR CAPACITY \n'
    with open('Results for '+str(TPd)+' Kg per hr.txt','w') as results:
        results.write(Title.center(150))
        for Parameter in Parameters:
            results.write('\n')
            results.write(Parameter.center(150).upper() + '\n\n')
            Parameter = Parameters[Parameter]
            for i in Parameter:
                if type(Parameter[i][0]) in (float, np.float64):
                    a = i + ' is' + ' '*(76-len(i) + 25) + str(round(Parameter[i][0], 4)) + Parameter[i][1]
                    results.write(a.title() + '\n')
                    print(a)
                else:
                    b = i + ' is' + ' '*(76-len(i) + 25) + str(Parameter[i][0]) + Parameter[i][1]
                    results.write(b.title() + '\n')
                    print(b)
